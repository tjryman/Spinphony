...
import { buildSpotifyPlaylist, buildAppleMusicPlaylist, buildDeezerPlaylist } from './services/playlistBuilder';
...
export default function App() {
  ...
  function selectPlatform(p: Platform) {
    setPlatform(p);
    // Deezer needs no auth — go straight to build
    if (p === 'deezer') {
      setAppState('build');
    } else {
      setAppState('connect');
    ...
    try {
      const result =
        config.platform === 'spotify'
          ...
          : config.platform === 'deezer'
          ? await buildDeezerPlaylist(config, setProgress)
        ...
        {appState === 'select-platform' && (
          <div className="flex flex-col items-center gap-8 py-12">
            ...
            <div className="flex flex-col gap-4 w-full max-w-lg">
              {/* Deezer — free, no account */}
              <button
                onClick={() => selectPlatform('deezer')}
                className="flex items-center gap-4 p-5 rounded-2xl border border-emerald-500/40 bg-emerald-500/5 hover:border-emerald-500/70 hover:bg-emerald-500/10 transition-all group"
              >
                <div className="w-14 h-14 rounded-2xl bg-emerald-500/20 flex items-center justify-center group-hover:bg-emerald-500/30 transition-colors shrink-0">
                  <svg viewBox="0 0 24 24" className="w-7 h-7 fill-emerald-400">
                    <path d="M19.032 0H4.968A4.968 4.968 0 000 4.968v14.064A4.968 4.968 0 004.968 24h14.064A4.968 4.968 0 0024 19.032V4.968A4.968 4.968 0 0019.032 0zM7.2 15.6H4.8V8.4h2.4v7.2zm4.8 0H9.6V6H12v9.6zm4.8 0h-2.4V10.8h2.4v4.8z"/>
                  </svg>
                </div>
                <div className="text-left">
                  <div className="flex items-center gap-2">
                    <p className="font-bold text-white">Deezer</p>
                    <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-medium">Free · No account needed</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5">Generate playlists instantly with accurate BPMs — no login required</p>
                </div>
              </button>

              {/* Spotify + Apple Music — save to library */}
              <p className="text-xs text-gray-600 text-center">— or connect your streaming library —</p>

              <div className="grid sm:grid-cols-2 gap-3">
                <button
                  onClick={() => selectPlatform('spotify')}
                  className="flex flex-col items-center gap-3 p-5 rounded-2xl border border-spin-border bg-spin-card hover:border-[#1DB954]/50 hover:bg-[#1DB954]/5 transition-all group"
                >
                  <div className="w-12 h-12 rounded-2xl bg-[#1DB954]/20 flex items-center justify-center group-hover:bg-[#1DB954]/30 transition-colors">
                  ...
                  <div className="text-center">
                    <p className="font-semibold text-white text-sm">Spotify</p>
                    <p className="text-xs text-gray-500 mt-0.5">Save to your library</p>
                  </div>
                </button>

                <button
                  onClick={() => selectPlatform('apple-music')}
                  className="flex flex-col items-center gap-3 p-5 rounded-2xl border border-spin-border bg-spin-card hover:border-pink-500/50 hover:bg-pink-500/5 transition-all group"
                >
                  ...
                  <div className="text-center">
                    <p className="font-semibold text-white text-sm">Apple Music</p>
